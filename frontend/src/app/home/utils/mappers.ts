...
export const mapCheckedToType = <T, K>(
  ...
  return checked
    .map((checked, i) => ({ checked, type: total[i] }))
...
export const paramsMapToReviewSearchDto = (
  params: ParamMap
): ReviewSearchDto => {
  const genres = params
    .getAll('gameGenres')
  ...
  const excludedAuthors = params
    .getAll('excludedAuthors')
  ...
  const platforms = params.getAll('platforms').filter(isPlatform);
  const difficulties = params.getAll('difficulty').filter(isDifficulty);
  const completed = isBoolean(params.get('completed'))
    ? params.get('completed') === 'true'
    : undefined;
  const replayable = isBoolean(params.get('replayable'))
    ? params.get('replayable') === 'true'
    : undefined;
...
export const paramsMapToGameSearchDto = (params: ParamMap): GameSearchDto => {
  ...
  const excludeNoRating = isBoolean(params.get('excludeNoRating'))
    ? params.get('excludeNoRating') === 'true'
    : undefined;
  const search = params.get('search') !== null ? params.get('search') : null;
  const searchNotNull = search !== null ? search : undefined;
...
export const paramsMapToUserSearchDto = (
  params: ParamMap,
  defaultSort = '',
  defaultDirection = ''
): UserSearchDto => {
...
export const gameSearchForSubmitReviewParamsToDto = (
  params: ParamMap,
