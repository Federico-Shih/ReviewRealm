...
export class GameFormComponent implements OnInit {
  ...
  gameForm = this.formBuilder.group({
    ...
    developer: new FormControl('', [Validators.required, Validators.minLength(1),
      Validators.maxLength(50)]),
    publisher: new FormControl('', [Validators.required, Validators.minLength(1),
      Validators.maxLength(50)]),
  ...
  ngOnInit(): void {
    ...
    if (this.editGame !== null) {
      ...
      this.gameForm.get('_image')?.setValue(this.translateService.instant('game-image'));
      this.gameForm.get('releaseDate')?.setValue(this.editGame.publishDate.toISOString());
      ...
      if (gameGenres !== undefined) {
        ...
        this.genres.forEach((genre, index) => {
          const isSelected = gameGenres.find((selection) => selection.id === genre.id) !== undefined;
  ...
  createGame() {
    ...
    if(selectedGenres.every( (value:boolean) => !value)){
    ...
    output.append('name', this.gameForm.get('name')?.value!!);
    output.append('description', this.gameForm.get('description')?.value!!);
    output.append('developer', this.gameForm.get('developer')?.value!!);
    output.append('publisher', this.gameForm.get('publisher')?.value!!);
    const isoDate = new Date(this.gameForm.get('releaseDate')?.value!!).toISOString();
    output.append('releaseDate', isoDate.split('T')[0]);
    if(this.temporaryImage !== null){ //Caso en edit donde no cambiaste la imagen
      output.append('image', this.temporaryImage!!);
    }
    const output_genres = mapCheckedToType((selectedGenres as boolean[]),  this.genres as Genre[],(genre) => genre.id);
