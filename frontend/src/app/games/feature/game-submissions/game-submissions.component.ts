...
export class GameSubmissionsComponent implements OnInit{
  ...
  reloadSubmisions() {
    this.gameService.getGames(`${environment.API_ENDPOINT}/games/`,{suggested: true}).subscribe( {
      next: submissions => {this.submissions$.next(submissions); console.log(submissions);},
      error: () => {},
  ...
  nextPage(event:Event) {
    if(event.target instanceof HTMLElement
      && (event.target.scrollTop / (event.target.scrollHeight - event.target.clientHeight)) > GameSubmissionsComponent.PERCENTAGE_SCROLLED &&
      this.submissions$.value.links.next && !this.loadingInfinite$.value && !this.loading$.value
    ){
      this.loadingInfinite$.next(true)
      this.gameService.getGames(this.submissions$.value.links.next,{suggested: true})
        .subscribe({
  ...
  handleSubmission(event: GameSubmissionEvent) {
    if(event.game.links.self) {
      this.gameService.handleGameSubmission(event.game.links.self,{accept: event.accept}).subscribe(() =>{
        this.loading$.next(true);
        this.snackBar.open(
          this.translate.instant('submission.handled'), undefined,
          {
  ...
  openSearch(){
    const dialogRef = this.dialog.open(GameSubmissionSearchDialog, {
      maxHeight: '100vh',
      height: '100%',
      minWidth: '80%',
    });
...
export class GameSubmissionSearchDialog{
  ...
  submitSearch() {
    if (this.search.value) {
      this.loading$.next(true);
      this.gameService.getGames(`${environment.API_ENDPOINT}/games`, {search: this.search.value}).subscribe(
        next => {this.games$.next(next);},
        () => {},
