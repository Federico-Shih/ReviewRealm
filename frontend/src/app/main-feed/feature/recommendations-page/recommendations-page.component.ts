...
export class RecommendationsPageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loggedInUser$.subscribe((user) => {
      if (user !== null && user.links.recommendedGames) {
        this.gameRecommendationLink$.next(user.links.recommendedGames + '&page=' + position);
      }
    });
  }
  ...
  changeLink(link: string, next: boolean) {
    ...
    if (current) {
      if (next) this.router.navigate([], {queryParams: {position: +current + 1}});
      else this.router.navigate([], {queryParams: {position: +current - 1}});
    }
  }
}
