...
export const TOTAL_PAGES_HEADER = 'X-Reviewrealm-TotalPages';
export const TOTAL_ELEMENTS_HEADER = 'X-Reviewrealm-TotalElements';
...
export const splitLinks = (links: string): Map<string, string> => {
...
export const paginatedResponseMapper =
  <T, K>(fromResponse: (param: T) => K) =>
  (httpResponse: HttpResponse<T[]>): Paginated<K> => {
    const paginatedResponse =
      httpResponse.status === 204 ? [] : (httpResponse.body as T[]);
    ...
    const totalElements =
      elementsHeader !== null ? parseInt(elementsHeader) : 0;
...
export const paginatedObjectMapper = <T>(
  httpResponse: HttpResponse<unknown>,
  objectArray: T[]
): Paginated<T> => {
  const paginatedResponse =
    httpResponse.status === 204 ? [] : (objectArray as T[]);
  const headers = httpResponse.headers;
  const pagesHeader = headers.get(TOTAL_PAGES_HEADER);
  const elementsHeader = headers.get(TOTAL_ELEMENTS_HEADER);
  const totalPages = pagesHeader !== null ? parseInt(pagesHeader) : 0;
...
export const queryMapper = <T extends Record<string, unknown>>(
  query: T,
  url: string
): string => {
