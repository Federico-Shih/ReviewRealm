...
export class AuthenticationService {
  ...
  login({ username, password }: AuthenticationDto): Observable<User | never> {
    return this.http
      .get<unknown>(AuthenticationService.AUTHENTICATION_ENDPOINT, {
        headers: {
  ...
  enableUser(email: string, token: string) {
    return this.http
      .get(AuthenticationService.AUTHENTICATION_ENDPOINT, {
        headers: {
   ...
   */
  private getAuthenticatedUserFromHeader(
    headers: HttpHeaders
    ...
    return this.userService
      .getUsers(AuthenticationService.AUTHENTICATION_ENDPOINT, {
        id: payload.id,
      })
      .pipe(
        map(users => {
