...
export class EnumsService {
  ...
  getMissionsComplete(url: string): Observable<MissionComplete[]> {
    return this.http
      ...
      .pipe(
        mergeMap(response => {
          if (response.body === null) return of([]);
          return forkJoin(
            response.body.map(progress =>
              this.getMission(progress.mission).pipe(
                map(
                  mission =>
                    new MissionComplete(
                      MissionProgress.fromResponse(progress),
                      mission
                    )
                )
              )
  ...
  getNotificationComplete(url: string): Observable<NotificationComplete[]> {
    return this.http
      .get<NotificationValueResponse[]>(url, {
        responseType: 'json',
        observe: 'response',
      })
      .pipe(catchError(exceptionMapper))
        ...
        mergeMap(response => {
          ...
          return forkJoin(
            response.body.map(value =>
              this.getNotificationType(value.links.notification).pipe(
                map(
                  type =>
                    new NotificationComplete(
                      NotificationType.fromResponse(type),
                      value
                    )
                )
              )
  ...
  getNotificationTypes(url: string): Observable<NotificationType[]> {
    return this.http
      .get<NotificationTypeResponse[]>(url, {
        responseType: 'json',
        observe: 'response',
      })
