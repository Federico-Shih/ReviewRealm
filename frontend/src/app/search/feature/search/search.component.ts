...
export class SearchComponent {
  games$ = this.route.queryParamMap
    .pipe(
      switchMap(param =>
  ...
  reviews$ = this.route.queryParamMap
    .pipe(
      switchMap(param =>
  ...
  users$ = this.route.queryParamMap
    .pipe(
      switchMap(param =>
  ...
  combinedResults$: Observable<{
    ...
    hasResults: boolean;
  }> = combineLatest([this.games$, this.reviews$, this.users$])
    .pipe(map(([games, reviews, users]) => (
      {
        games,
        reviews,
        users,
        hasResults: games.totalPages > 0 || reviews.totalPages > 0 || users.totalPages > 0
      }
    )));
