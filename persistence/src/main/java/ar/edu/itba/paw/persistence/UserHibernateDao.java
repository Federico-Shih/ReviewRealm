...
public class UserHibernateDao implements UserDao, PaginationDao<UserFilter> {
    ...
    public Paginated<User> findAll(Page page, UserFilter userFilter, Ordering<UserOrderCriteria> ordering) {
        ...
        Query nativeQuery = em.createNativeQuery("SELECT mainuserid FROM ("+ "SELECT distinct u.id as mainuserid,u.level,u.reputation, coalesce((SELECT count(distinct f.userid) FROM followers as f WHERE f.following=u.id GROUP BY f.following), 0) as follower_count FROM " + toTableString(userFilter) + queryBuilder.toQuery()  + ") as users" + toOrderString(ordering, true));
